...
p <- ggplot(j1, aes(x=publication_year, y=n, fill=language_all, colour=language_all)) +
  geom_point() +
  stat_smooth(method = 'loess')   + theme_classic() + grids(linetype = "dashed") +
  facet_wrap(~cat, scales = "free") +
  # labs(title="theology grand categories; top-3 language_all") +
  labs(x = "Publication year", y = "Title count (N)", color="", fill="") + 
  theme_comhis("discrete", base_size=20) +
  scale_color_manual(values=mycols) +
  scale_fill_manual(values=mycols) 
  

CairoJPEG("Figure10.jpg", width=1.3*500, height=1*500, quality=100)
print(p)
dev.off()
