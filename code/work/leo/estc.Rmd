...
```{r reprints, echo=FALSE, message=FALSE, warning=FALSE, out.width="25%", fig.width=6, fig.height=6}
  ...
  for (doct in doctypes){
      ...
      p <- ggplot(dfss, aes(x = publication_year, y = n)) +
           geom_smooth() +
       	   ...
       	   labs(title="Title count (N)", subtitle=paste(doct, lang, sep = "/"))
      print(p)
      ...
      p <- ggplot(dfss, aes(x = publication_year, y = n)) +
           geom_smooth() +
       	   theme_comhis("discrete", base_size=20) +
       	   labs(x = "Publication year", y = "Title count (N)") +
       	   labs(title="Title count (N)", subtitle=paste("all", lang, sep = "/"))
      print(p)
}
