...
for (country in unique(df$publication_country)) {
  ...
  p <- top_plot(dfs$language_primary, ntop = ntop) +
       labs(title = "Top languages by title count", subtitle = paste(country, catalog, sep = " / ")) +
       scale_y_log10() +
...
```{r top_language_by_town, echo=FALSE, message=FALSE, warning=FALSE, out.width="25%"}
# Combine language data from catalogs
df <- NULL
for (ctl in names(catalogs)) {
  print(ctl)
...
# For each country, show top languagues
ntop <- 5
for (i in seq_along(selected.towns)) {
  ...
  library(ggplot2)  
  p <- top_plot(dfs$language_primary, ntop = ntop) +
       labs(title = "Top languages", subtitle = paste(town, ctl, sep = " / ")) +
       scale_y_log10() +
       theme_comhis("discrete")
