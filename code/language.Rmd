...
for (ctl in setdiff(names(catalogs), "stcn")) {
  tmp <- catalogs[[ctl]][, c("language_primary", "publication_country", "publication_place", "publication_decade")]
  tmp$catalog <- rep(ctl, nrow(tmp))
  df <- dplyr::bind_rows(df, tmp)
}
...
```{r top_language_by_town_by_decade, echo=FALSE, message=FALSE, warning=FALSE, out.width="33%", fig.width=10, fig.height=7}
...
for (i in seq_len(nrow(selected.towns))) {
  ...
  dfs <- dfs %>% group_by(publication_decade, publication_place, language_primary) %>%
   ...
   dfs$language_primary <- droplevels(factor(dfs$language_primary, levels = langs))
  ...
  p <- ggplot(dfs, aes(x = publication_decade, y = n)) +
         geom_bar(aes(fill = language_primary), position = "stack", stat = "identity") + 
         labs(title = town, subtitle = ctl, y = "Title count (n)") +
         scale_y_log10() +
         theme_comhis("continuous", base_size=30)
