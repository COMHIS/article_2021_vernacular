...
for (i in seq_len(nrow(selected.towns))) {
  ...
  dfs$language_primary <- compress_field(dfs$language_primary, topn = 4, rest = "Other")
  ...
  # Line plot version
  #p2 <- ggplot(dfs, aes(x = publication_decade, y = n)) +
  #       geom_smooth(aes(color = language_primary, fill = language_primary)) +
